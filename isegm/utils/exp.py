...
def init_experiment(args, model_name):
    ...
    cfg = load_config(config_path, model_path)
    ...
    exp_parent_path = experiments_path / 'train_logs' / '/'.join(ftree)
    exp_parent_path.mkdir(parents=True, exist_ok=True)

    if cfg.resume_exp:
        exp_path = find_resume_exp(exp_parent_path, cfg.resume_exp)
    else:
        last_exp_indx = find_last_exp_indx(exp_parent_path, cfg.exp_name)
...
def find_last_exp_indx(exp_parent_path, specified_exp_name):
    indx = 0
    for x in exp_parent_path.iterdir():
        if not x.is_dir():
            continue
        if x != exp_parent_path / (specified_exp_name + '_' + str(x).split('_')[-1]):
            continue
        # exp_name = x.stem
        exp_name = str(x).split('/')[-1]
