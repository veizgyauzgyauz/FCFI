...
class Clicker(object):
    ...
    def make_next_click(self, pred_mask, click_ind=0, img_name=None):
    ...
    def __len__(self, clicks_limit=None):
        return len(self.clicks_list[:clicks_limit])

    def get_click_label_map(self, h, w, clicks_limit=None):
        click_label_map = -1 * np.ones((h, w))
        range_limit = clicks_limit if clicks_limit is not None and clicks_limit <= len(self.clicks_limit) else len(self.clicks_list)
        for i in range(range_limit):
            y, x = self.clicks_list[i].coords
            is_positive = self.clicks_list[i].is_positive
            click_label_map[y, x] = 1 if is_positive else 0
        return click_label_map
    ...
    def _get_next_click(self, pred_mask, padding=True):
        ...
        assert coords_y[0] > -1e-5
        assert coords_x[0] > -1e-5
...
class Click:
    def __init__(self, is_positive, coords, indx=None):
        ...
        y = int(coords[0]) if isinstance(coords[0], float) else coords[0]
        x = int(coords[1]) if isinstance(coords[1], float) else coords[1]
        self.coords = (y, x)
