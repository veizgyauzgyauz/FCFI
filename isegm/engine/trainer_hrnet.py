...
class ISTrainer(object):
    ...
    def training(self, epoch):
        ...
        for i, batch_data in enumerate(tbar):
            ...
            if self.is_master:
                ...
                vis_out = torch.sigmoid(outputs['updated_feedback1'][-1].detach()[vis_b,0]).cpu().numpy()
                plt.subplot(2, 5, 7)
                plt.imshow(vis_out, cmap='jet')
                plt.colorbar()
                ...
                plt.subplot(2, 5, 8)
                plt.imshow(outputs['valid_masks'][-1][vis_b,0].detach().cpu().numpy(), cmap='jet')
                plt.colorbar()
                plt.title('Mask')

                vis_out = torch.sigmoid(outputs['updated_feedback2'][-1].detach()[vis_b,0]).cpu().numpy()
                plt.subplot(2, 5, 9)
                plt.imshow(vis_out, cmap='jet')
                plt.colorbar()
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            for i in range(len(output['updated_feedback1'])):
                loss = self.add_loss('feedback1_loss', loss, losses_logging, validation,
                                    lambda: (output['updated_feedback1'][i], batch_data['instances'], output['valid_masks'][i]))
            for i in range(len(output['updated_feedback2'])):
                loss = self.add_loss('feedback2_loss', loss, losses_logging, validation,
                                    lambda: (output['updated_feedback2'][i], batch_data['instances']))
...
def get_next_points(pred, gt, points, click_indx, pred_thresh=0.49):
    ...
    new_points = torch.tensor(new_points).to(device) # (batch_size, 2)
...
def load_weights(model, path_to_weights):
    ...
    filtered_dict = {k: v for k, v in new_state_dict.items() if k in current_state_dict}
