    ...
    def _make_fuse_layers(self):
        ...
        for i in range(num_branches if self.multi_scale_output else 1):
            ...
            for j in range(num_branches):
                if j > i: # ↗, change channel number and upsample
                ...
                else: # ↘, (change channel number and) downsample
                    conv3x3s = []
                    for k in range(i - j):
                        if k == i - j - 1:
...
class HighResolutionNet(nn.Module):
    def __init__(self, width, num_classes, aux_ch, ocr_width=256, mid_ch=32, small=False,
        ...
        self.correction_module = FocusedCorrectionModule(
            in_ch=last_inp_channels, aux_ch=aux_ch, mid_ch=mid_ch, norm_layer=norm_layer, expan_ratio=expansion_ratio
        )
        self.fusion_module = CollaborativeFeedbackFusionModule(
            in_ch=last_inp_channels, mid_ch=mid_ch, norm_layer=self.norm_layer
        )
    ...
    def _make_correction_modules(
        self, num_channels_cur_layer, aux_ch,
        norm_layer, mid_channels, expan_ratio=0.2,
    ):
        num_branches_cur = len(num_channels_cur_layer)
        ...
        for i in range(num_branches_cur):
            ffc_modules.append(
                FocusedFeedbackCorrectionModule(
                    in_ch=num_channels_cur_layer[i], aux_ch=aux_ch,
                    mid_ch=mid_channels[i], norm_layer=norm_layer,
                    expan_ratio=expan_ratio
                )
            )
        
        return nn.ModuleList(ffc_modules)
    
    def _make_fusion_modules(
        self, num_channels_cur_layer, norm_layer, mid_channels
    ):
        ...
        for i in range(num_branches_cur):
            cff_modules.append(
                CollaborativeFeedbackFusionModule(
                    in_ch=num_channels_cur_layer[i],
        ...
        return nn.ModuleList(cff_modules)
    ...
    def forward(self, input, click_encoding, new_points, feedback, gate, additional_features=None, is_training=True):
        feats = self.compute_hrnet_feats(input, additional_features)
        
        updated_feedback1, valid_mask = self.correction_module(input, click_encoding, feats, feedback, new_points, is_training=is_training)
        feats, updated_feedback2 = self.fusion_module(feats, updated_feedback1, gate, self.update_num)
            
        if self.ocr_width > 0:
            out_aux = self.aux_head(feats) # (B, 1, H // 4, W // 4)
            feats = self.conv3x3_ocr(feats) # (B, 128, H // 4, W // 4)

            context = self.ocr_gather_head(feats, out_aux) # (B, 128, 1, 1)
            feats = self.ocr_distri_head(feats, context) # (B, 128, H // 4, W // 4)
            out = self.cls_head(feats) # (B, 1, H // 4, W // 4)
            return [out, out_aux, [updated_feedback1], [valid_mask], [updated_feedback2]]
        else:
            return [self.cls_head(feats), None, [updated_feedback1], [valid_mask], [updated_feedback2]]

    def compute_hrnet_feats(self, image, additional_features):
        x = self.compute_pre_stage_features(image, additional_features)
        x = self.layer1(x)
        ...
        return self.aggregate_hrnet_features(y_list)
