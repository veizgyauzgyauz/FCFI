...
class ObjectAttentionBlock2D(nn.Module):
    ...
    def forward(self, x, proxy):
        ...
        query = self.f_pixel(x).view(batch_size, self.key_channels, -1) # (B, C, HW)
        query = query.permute(0, 2, 1) # (B, HW, C)
        key = self.f_object(proxy).view(batch_size, self.key_channels, -1) # (B, C, K)
        value = self.f_down(proxy).view(batch_size, self.key_channels, -1) # (B, C, K)
        ...
        context = context.view(batch_size, self.key_channels, *x.size()[2:]) # (B, C, H, W)
