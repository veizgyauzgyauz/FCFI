...
class MaskedNormalizedFocalLossSigmoid(nn.Module):
    def __init__(self, axis=-1, alpha=0.25, gamma=2, max_mult=-1, eps=1e-12,
                 from_sigmoid=False, detach_delimeter=True,
                 batch_axis=0, weight=None, size_average=True,
                 ignore_label=-1):
        super(MaskedNormalizedFocalLossSigmoid, self).__init__()
        self._axis = axis
        self._alpha = alpha
        self._gamma = gamma
        self._ignore_label = ignore_label
    ...
    def forward(self, pred, label, mask):
        one_hot = label > 0.5
        sample_weight = torch.logical_and(label != self._ignore_label, mask.long() == 1)
...
class CrossEntropyLoss(nn.Module):
    def __init__(self, weight=None, batch_axis=0, ignore_label=-1):
    ...
    def forward(self, pred, label):
        ...
        label = label.long()
        loss = F.cross_entropy(pred, label, ignore_index=self._ignore_label, reduction='mean')
