...
class DistMaps(nn.Module):
    ...
    def get_coord_features(self, points, batchsize, rows, cols, designated_radius=-1):
        '''
        coords: batch_size*2*h*w
        '''
        if self.cpu_mode:
            coords = []
            ...
            if not self.use_disks:
                if designated_radius < 0:
                    coords.div_(self.norm_radius * self.spatial_scale)
                else:
                    coords.div_(designated_radius * self.spatial_scale)
            coords.mul_(coords)
        ...
        if self.use_disks:
            if designated_radius < 0:
                ...
                coords = (coords <= (designated_radius * self.spatial_scale) ** 2).float()
        else:
            coords.sqrt_().mul_(2).tanh_()
            
        return coords

    def forward(self, x, coords, designated_radius=-1):
        '''
        coords: batch_size * point_num * 3 (y, x, 0/1)
        '''
        return self.get_coord_features(coords, x.shape[0], x.shape[2], x.shape[3], designated_radius)
