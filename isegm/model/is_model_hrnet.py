...
class ISModel(nn.Module):
    ...
    def forward(self, image, points, gate):
        ...
        gauss_dist_maps = self.get_coord_features(image, points)

        if self.rgb_conv is not None:
            x = self.rgb_conv(torch.cat((image, gauss_dist_maps), dim=1))
            outputs = self.backbone_forward(x, prev_mask, gate)
        else:
            coord_features = self.maps_transform(gauss_dist_maps)
            outputs = self.backbone_forward(image, prev_mask, gate, coord_features)

        outputs['instances'] = nn.functional.interpolate(outputs['instances'], size=image.size()[2:],
                                                         mode='bilinear', align_corners=True)
        outputs['updated_feedback'] = nn.functional.interpolate(outputs['updated_feedback'], size=image.size()[2:],
                                                         mode='bilinear', align_corners=True)
    ...
    def get_coord_features(self, image, points):
        if self.clicks_groups is not None:
