...
from isegm.engine.trainer_hrnet import ISTrainer
...
def init_model(cfg):
    ...
    model = HRNetModel(width=18, ocr_width=48, mid_ch=cfg.mid_ch, with_aux_output=True, use_leaky_relu=True,
                       update_num=cfg.update_num, expansion_ratio=cfg.expansion_ratio,
                       use_rgb_conv=False, use_disks=True, norm_radius=5,
                       with_prev_mask=cfg.with_prev_mask)
...
def train(model, cfg, model_cfg):
    ...
    loss_cfg.feedback1_loss_weight = 0.2
    loss_cfg.feedback2_loss = NormalizedFocalLossSigmoid(alpha=0.5, gamma=2)
    loss_cfg.feedback2_loss_weight = 0.2
